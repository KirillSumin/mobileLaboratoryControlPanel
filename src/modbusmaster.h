#include <QBitArray>
//#include <QDebug> //убрать в конечной версии
...
public:
    ...
    QMap<ModbusMaster::ModbusError, QString> err_to_qstr = { { NO_ERROR, "NO_ERROR" },
    ...
    QPair<QString[2], ModbusMaster::ModbusError> readID();
...
private:
    ...
    QVariant LAB_PORT = (QVariant) "COM1";
    QVariant LAB_BAUDS = (QVariant)19200;
    QVariant LAB_STOP_BITS = (QVariant)1;
    QVariant LAB_DATA_BITS = (QVariant)8;
    QVariant LAB_PARITY = (QVariant)QSerialPort::NoParity;

    int LAB_RESPONSE_TIME = 500; //время таймаута ожидания одного ответа от slave modbus (min 10)

    //время ожидания сигнала от modbus slave, сигнал появляется при получении
    //ошибки или верного результата
    //общее время ожидания ответа от modbus qt класса (+500 для отработки неожиданной задержки)
    int MODBUS_QT_LIBRARY_RESPONSE_TIME = LAB_RESPONSE_TIME * MODBUS_TRY_SEND_COUNT + 500;
